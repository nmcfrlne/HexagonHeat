...
public class HexagonCamera : MonoBehaviour {

	

	public float followSpeed = 3; //Speed ​​at which the camera follows us
	public float mouseSpeed = 0.2f; //Speed ​​at which we rotate the camera with the mouse
	...
	public float turnSmoothing = .1f; //Smooths all camera movements (Time it takes the camera to reach the rotation indicated with the joystick)
	...
	{ //Function that rotates the camera correctly
		...
		tiltAngle -= smoothY * targetSpeed / Time.deltaTime; //Update the angle at which the camera will move
		...
		lookAngle += smoothX * targetSpeed / Time.deltaTime; //Updates the rotation angle in y smoothly
	...
	public static HexagonCamera singleton; //You can call CameraManager.singleton from other script (There can be only one)
	void Awake()
	{
		if (view.IsMine)
		{
...
//void Update()
//{
//	moveCharacter(Input.GetAxis("Vertical"), Input.GetAxis("Horizontal"));

//	float rightStickHorizontalRotation = Input.GetAxis("Right Stick X") * rightStickSensitivity * Time.deltaTime;
//	transform.Rotate(0, rightStickHorizontalRotation, 0);
//	float mouseHorizontalRotation = Input.GetAxis("Mouse X") * mouseSensitivity;
//	transform.Rotate(0, mouseHorizontalRotation, 0);

//	VerticalRotation -= Input.GetAxis("Mouse Y") * mouseSensitivity;
//	VerticalRotation -= Input.GetAxis("Right Stick Y") * rightStickSensitivity * Time.deltaTime;
...
//{
//	moveCharacter(Input.GetAxis("Vertical"), Input.GetAxis("Horizontal"));
//	//moveCamera(Input.GetAxis("Vertical Camera"), Input.GetAxis("Horizontal Camera"));

//	float horizontalRotation = Input.GetAxis("Horizontal Camera") * sensitivity;
//	transform.Rotate(0, horizontalRotation, 0);

//	verticalRotation -= Input.GetAxis("Vertical Camera") * sensitivity;
//	verticalRotation = Mathf.Clamp(verticalRotation, -90f, 90f);
//	Camera.main.transform.localRotation = Quaternion.Euler(verticalRotation, 0, 0);
//}